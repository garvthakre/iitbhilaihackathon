...
class PageTwo(tk.Frame):
    ...
    def next_foo(self):
        selected_name = self.menuvar.get()  # Get selected name
        if selected_name == "None" or not selected_name:
            messagebox.showerror("ERROR", "Please select a name!")
            return
        
        # Set the active_name in the controller
        self.controller.active_name = selected_name
        print(f"Active name set to: {self.controller.active_name}")  # Debugging line
...
class PageFour(tk.Frame):
    ...
    def openwebcam(self):
        active_name = self.controller.active_name  # Get active_name from controller
        if not active_name:
            messagebox.showerror("Error", "Please select the name first!")
            return
        print(f"Recognizing face for: {active_name}")  # Debugging line
        main_app(active_name)  # Pass active_name to the face recognition function
